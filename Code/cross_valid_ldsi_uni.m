...
function err = cross_valid_ldsi_uni(Y, m_upper, n_fold, timePoints, varargin)
...
%             while ~is_fit
%                 try
                    Ph         = lds_uni_latent(Y_train, m, 'timePoint', timePoints, varargin{:});
%                     is_fit     = true;
%                 catch
%                     is_fit     = false;
%                 end
...
%             end            
            [err(m, curr_fold), ~, ~]  = loo (Y_test, Ph, [0, timePoints, T]);
        end        
    end