...
function Ph = em_lds_uni_latent(Yk_t, m, cyc, tol, C, R, A, Q, x0, Q0, timePoint, is_fix_C, is_fix_A)
...
%         figure;plot(squeeze(Xk_t))
        ...
        if is_fix_A
            A_now    = M_step_uni_A (sum(Q_xx_ss,3),sum(Q_xx_ts,3));
        end
        
        if is_fix_C
            C_now    = M_step_uni_C (sum(Q_yx_tt,3),sum(Q_xx_tt,3));
        end
        ...
        for nt_now = 1: nt            
            ...
            if ~is_fix_C
                ...
                [R_now, ~]        = M_step_R (C_now, Q_yy_tt(:,:,nt_now), Q_yx_tt(:,:,nt_now), length_T);
            end
            C(:,:,nt_now)     = C_now;
            R(:,:,nt_now)     = R_now;
            ...
            if ~is_fix_A
                ...
                [Q_now, ~]        = M_step_Q (A_now, Q_xx_ts(:,:,nt_now), Q_xx_uu(:,:,nt_now), length_T - K);
            end
            A(:,:,nt_now)     = A_now;
            Q(:,:,nt_now)     = Q_now;  
        end
